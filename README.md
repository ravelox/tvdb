...
Each run increments a local `.docker-build-number` counter and tags the image with the semantic package version (for example `1.8.11`) plus a numeric suffix such as `1.8.11.7`. Set the optional `APP_VERSION`/`BUILD_NUMBER` build arguments (or the matching environment variables consumed by `docker-compose.yaml`) if you need to override either value manually.
